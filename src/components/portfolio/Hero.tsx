...
export function Hero({ profile, portraitUrl }: { profile: Record<string, any>; portraitUrl?: string }) {
  ...
  const src = portraitUrl || portraitAsset.url;
  return (
    <section id="top" className="relative overflow-hidden pt-28 pb-16 sm:pt-32 lg:pt-40 lg:pb-24">
      ...
      <div className="relative mx-auto grid max-w-7xl grid-cols-1 items-center gap-12 px-4 sm:px-6 lg:grid-cols-[1.1fr_0.9fr] lg:gap-16 lg:px-8">
        <div className="animate-fade-up">
          ...
          <p className="mt-5 text-lg font-semibold text-gradient-brand">{roles.join(" · ")}</p>
          <p className="mt-4 max-w-xl text-base text-muted-foreground sm:text-lg">{profile.tagline}</p>

          <div className="mt-8 flex flex-wrap items-center gap-3">
            <a href="#contact" className="inline-flex items-center gap-2 rounded-full bg-gradient-brand px-5 py-3 text-sm font-semibold text-brand-foreground shadow-xl shadow-brand/25 transition hover:opacity-95">
              <CalendarCheck className="h-4 w-4" /> Book an appointment
            </a>
            <a href={`mailto:${profile.email ?? ""}`} className="inline-flex items-center gap-2 rounded-full border border-border bg-surface-elevated/60 px-5 py-3 text-sm font-semibold text-foreground backdrop-blur transition hover:bg-accent">
              <Mail className="h-4 w-4" /> Get in touch
            </a>
            <a href="#experience" className="inline-flex items-center gap-1.5 rounded-full px-3 py-3 text-sm font-semibold text-muted-foreground transition hover:text-foreground">
          ...
          <dl className="mt-10 flex flex-wrap items-center gap-x-6 gap-y-3 text-sm text-muted-foreground">
            {profile.location && <div className="inline-flex items-center gap-2"><MapPin className="h-4 w-4 text-brand" /><span>{profile.location}</span></div>}
            {profile.phone && <div className="inline-flex items-center gap-2"><Phone className="h-4 w-4 text-brand" /><span>{profile.phone}</span></div>}
          </dl>
        </div>
...
function SplitName({ name }: { name: string }) {
  ...
  return (<>{parts.join(" ")} <span className="text-gradient-brand">{last}</span></>);
}
